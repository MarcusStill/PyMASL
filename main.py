...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.ui.pushButton_20.clicked.connect(self.main_open_sale_service)

    def main_open_sale_service(self) -> None:
        """
        Функция открывает форму продажи дополнительных услуг.
        ...
        sale_service: SaleServiceForm = SaleServiceForm()
        sale_service.show_form()
        sale_service.exec_()
...
class SaleServiceForm(QDialog):
    """Форма продажи доп.услуг"""

    def __init__(self):
        super().__init__()
        self.ui = Ui_Dialog_Sale_Service()
        self.ui.setupUi(self)
        self.ui.pushButton_12.clicked.connect(self.clearing_client_list)
        self.ui.pushButton_4.clicked.connect(self.close)
        self.ui.pushButton.clicked.connect(self.sale_service_search_clients)
        ...
        # Подключаем сигнал для изменения состояния checkBox_2
        self.ui.checkBox_2.toggled.connect(self.toggle_date_edit)
        # Включаем фильтрацию событий для отслеживания наведения мыши на ячейки
        self.ui.tableWidget_3.installEventFilter(self)
        # Подключаем обработчик событий для comboBox_4
        self.ui.comboBox_4.currentIndexChanged.connect(self.clear_line_edit)
        # Настройка ширины столбцов и скрытие некоторых столбцов для tableWidget_3
        self.ui.tableWidget_3.setColumnWidth(0, 250)
        self.ui.tableWidget_3.setColumnWidth(1, 15)
        self.ui.tableWidget_3.setColumnWidth(2, 300)
        self.ui.tableWidget_3.setColumnHidden(3, True)
        self.ui.tableWidget_3.setColumnHidden(4, True)
        self.ui.tableWidget_3.setColumnHidden(5, True)
        self.ui.tableWidget_3.setColumnHidden(6, True)
        self.ui.tableWidget_3.setColumnHidden(7, True)
        # Настройка ширины столбцов и скрытие некоторых столбцов для tableWidget_2
        self.ui.tableWidget_2.setColumnWidth(0, 300)
        self.ui.tableWidget_2.setColumnWidth(2, 15)
        self.ui.tableWidget_2.setColumnWidth(3, 25)
        self.ui.tableWidget_2.setColumnHidden(5, True)
    ...
    def update_totals(self):
        ...
        child_tickets = 0
        adult_tickets = 0
        for row_position in range(self.ui.tableWidget_2.rowCount()):
            quantity_item = self.ui.tableWidget_2.item(row_position, 1)
            price_item = self.ui.tableWidget_2.item(row_position, 2)
            name_item = self.ui.tableWidget_2.item(row_position, 0)
            if quantity_item and price_item and name_item:
                try:
    ...
    def on_spinbox_value_changed(self, spin_box, row_position):
        """Обработчик для сигнала valueChanged QSpinBox"""
        # Обновляем значение в ячейке QTableWidgetItem
        self.ui.tableWidget_2.item(row_position, 1).setText(str(spin_box.value()))
        self.update_totals()
    ...
    def on_cell_double_clicked(self, row, column):
        ...
        if column == 2:  # Проверяем, что столбец - это второй столбец (с индексом 2)
            ...
            windows.info_window(
                "Описание услуги",  # Заголовок окна
                full_text,  # Текст, который нужно отобразить
                "",  # Дополнительное поле, если нужно
            )
        else:
            # Передаем всю строку в tableWidget_2
            self.transfer_row_to_tableWidget_2(row)
    ...
    def filling_client_table_widget_4(
        ...
        self.ui.tableWidget.setItem(row, 0, QTableWidgetItem(last_name))
        self.ui.tableWidget.setItem(row, 1, QTableWidgetItem(first_name))
        self.ui.tableWidget.setItem(row, 2, QTableWidgetItem(str(phone)))
        self.ui.tableWidget.setItem(row, 3, QTableWidgetItem(str(id)))
        self.ui.tableWidget.setColumnHidden(3, True)

    @logger_wraps()
    def sale_service_search_clients(self) -> None:
        """
        Функция Выводит в tableWidget список найденных клиентов.
        ...
        if index == 2:
            ...
            logger.debug(search)
            for client in search:
                self.filling_client_table_widget_4(
                    client.last_name,
                    client.first_name,
                    client.phone,
                    client.id,
    ...
    def load_price_service_data(self) -> None:
        """
        Функция загружает данные из таблицы PriceService и отображает их в tableWidget_3.
