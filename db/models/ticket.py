...
class Ticket(Base):
    ...
    datetime: Mapped[DateTime] = mapped_column(
        DateTime, nullable=False, default=datetime.utcnow, comment="Дата и время"
    )
    arrival_time: Mapped[int | None] = mapped_column(
        SmallInteger, comment="Время пребывания"
    )
    talent: Mapped[int | None] = mapped_column(
    ...
    print: Mapped[int | None] = mapped_column(
        SmallInteger, comment="Печатать билет: 0-нет, 1 - да"
    )
    ...
    def __str__(self) -> str:
        return (
            f"{self.id} {self.client_age} {self.datetime} {self.arrival_time} {self.talent} {self.price} "
