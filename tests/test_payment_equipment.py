...
if __name__ == '__main__':
    ...
    def test_run_terminal_command_timeout(self):
        with patch("subprocess.Popen") as mock_popen, \
            ...
            mock_process = MagicMock()
            mock_process.communicate.side_effect = [subprocess.TimeoutExpired(cmd="", timeout=1), (b"", b"")]
            mock_popen.return_value = mock_process
    ...
    def test_process_terminal_error_conditions(self):
        with patch("modules.payment_equipment.handle_error"):
            ...
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_CARD_BLOCKED[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_CARD_LIMIT[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_BIOMETRIC_ERROR[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_ERROR_PIN_CODE[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_NO_CONNECTION_BANK[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_HARDWARE_ERROR[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_OPERATION_CANCEL[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_LACK_PAPER[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_SUM_LIMIT[0], None), 0)
    ...
    def test_process_terminal_error_more_conditions(self):
        with patch("modules.payment_equipment.handle_error"):
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_COMMAND_ERROR[0], None), 0)
            self.assertEqual(pe.process_terminal_error(pe.TERMINAL_PIN_PAD_ERROR[0], None), 0)

    def test_universal_terminal_operation_unsupported_operation_type(self):
        mock_signal = MagicMock()
        res = pe.universal_terminal_operation(pe.PAYMENT_ELECTRONIC, 100.0, mock_signal, operation_type=99)
        self.assertEqual(res, (0, 0))

    def test_universal_terminal_operation_offline(self):
        mock_signal = MagicMock()
        res = pe.universal_terminal_operation(pe.PAYMENT_OFFLINE, 100.0, mock_signal, operation_type=1)
        self.assertEqual(res, (1, 3))
        
    def test_universal_terminal_operation_offline_not_payment(self):
        mock_signal = MagicMock()
        res = pe.universal_terminal_operation(pe.PAYMENT_OFFLINE, 100.0, mock_signal, operation_type=2)
        self.assertEqual(res, (0, 0))
        
    def test_universal_terminal_operation_unknown_payment_type(self):
        mock_signal = MagicMock()
        res = pe.universal_terminal_operation(999, 100.0, mock_signal, operation_type=1)
        self.assertEqual(res, (0, 0))
